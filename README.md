...
The recommended production path uses the multi-platform image published with
build provenance for each GitHub release:

```bash
cp .env.example .env
# Replace every placeholder in .env.
docker compose -f docker-compose.ghcr.yml pull
docker compose -f docker-compose.ghcr.yml up --detach
docker compose -f docker-compose.ghcr.yml ps
curl --fail http://127.0.0.1:8080/readyz
```

`WEBVOUCH_SLACK_IMAGE_TAG` defaults to the exact release version in the supplied
`.env.example`. Keep that full version pinned in production. Contributors who
want to build the current source checkout locally can instead run:
...
|---|---:|---|---|
| `WEBVOUCH_SLACK_IMAGE_TAG` | No | Current release | Docker image tag used only by `docker-compose.ghcr.yml`. Pin a complete release version in production. |
...
Back up the named data volume, set `WEBVOUCH_SLACK_IMAGE_TAG` in `.env` to the
new complete version, and recreate the service from the published image:
...
The source-build Compose file remains available to contributors. Tagged releases publish the recommended multi-platform image to this repository's GitHub Container Registry package. Pin a complete version or image digest in production rather than relying on `latest`.
...
1. Update `version` in `package.json` and `package-lock.json`, the default image tag in `docker-compose.ghcr.yml`, and `WEBVOUCH_SLACK_IMAGE_TAG` in `.env.example`.
